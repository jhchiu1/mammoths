...
with open('pbdb_data.csv', 'r') as csvfile:
	...
	for row in reader:
		...
		mammoth_data.append([name, abd, abd_unit, lag, lng, state, county, comment])

with open('mammoth_data.csv', 'w') as csvfile:
	writer = csv.writer(csvfile, quoting=csv.QUOTE_NONUMERIC)
	writer.writerow(['name', 'abd', 'abd_unit', 'lag', 'lng', 'state', 'county', 'comment'])
